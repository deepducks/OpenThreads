...
const PLATFORM_HINTS: Record<string, string> = {
  ...
  discord:
    'e.g. vault:discord/bot-token or the environment variable name for your Discord bot token',
  telegram:
    'e.g. vault:telegram/bot-token or the environment variable for your Telegram Bot API token',
...
type WizardStep = 'platform' | 'credentials' | 'test';
              ...
              {testResult === 'idle' && (
                <>
                  <Text type="secondary" style={{ display: 'block', marginBottom: 16 }}>
                    Click &ldquo;Test Connection&rdquo; to validate your credentials reference before
                    saving.
                  </Text>
                  <Button
                    loading={testing}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
          <Button
            onClick={
              step === 'platform'
                ? handleClose
                : () => setStep(step === 'credentials' ? 'platform' : 'credentials')
            }
          >
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: Channel) => (
        <Space>
          <Button size="small" icon={<EditOutlined />} onClick={() => setEditChannel(record)}>
            Edit
          </Button>
          <Popconfirm

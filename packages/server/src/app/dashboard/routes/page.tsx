...
const RouteFlowCanvas = dynamic(() => import('./RouteFlowCanvas'), {
  ...
  loading: () => (
    ...
    >
      <Text type="secondary">Loading route editor&hellip;</Text>
    </div>
  ),
});
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.matchingRouteIds.length === 0) {
        ...
        messageApi.success(
          `${result.matchingRouteIds.length} route(s) matched — highlighted in canvas`,
        );
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        ...
        <Col>
          <Space>
            <Button icon={<ExperimentOutlined />} onClick={() => setTestPanelOpen(true)}>
              Test Route
            </Button>
            <Button
              type="primary"
      ...
      <Card title="Route Priority Order" loading={loading}>
        {routes.length === 0 ? (
          <Text type="secondary">
            No routes defined. Create one using &ldquo;New Route&rdquo;.
          </Text>
        ) : (
